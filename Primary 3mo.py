...
benchmark = ['QQQ']  # Must be in your price file
start = '2006-07-01'  # Include an extra previous month for dataframe calculations
start_real = '2007-01-01'  # Date to start calculations
end = '2022-12-31'  # Last day of the last year within the dataset
end_real = '2022-12-31'  # Date to end calculations
portfolio_value = 10000  # Amount in dollars for initial portfolio value
length = 63  # Length of quarter
...
if Q2_dates[-1] >= end_real:
    Q2_dates[-1] = end_real

if mid_dates[-1] >= end_real:
    mid_dates[-1] = end_real

if Q3_dates[-1] >= end_real:
    Q3_dates[-1] = end_real

if end_dates[-1] >= end_real:
    end_dates[-1] = end_real
...
for v, w, x, y, z in training_date_range:
    ...
    nyse_trading_date_range_Q1 = nyse.schedule(v, w)
    nyse_trading_date_range_index_Q1 = mcal.date_range(nyse_trading_date_range_Q1, frequency='1D')\
        .strftime('%Y-%m-%d')\
        .tolist()
    trading_start_dates.append(nyse_trading_date_range_index_Q1[0])
    trading_Q2_dates.append(nyse_trading_date_range_index_Q1[-1])

    # Configure range of H1
    nyse_trading_date_range_Q2 = nyse.schedule(w, x)
    nyse_trading_date_range_index_Q2 = mcal.date_range(nyse_trading_date_range_Q2, frequency='1D')\
        .strftime('%Y-%m-%d')\
        .tolist()
    trading_mid_dates.append(nyse_trading_date_range_index_Q2[-1])
    ...
    nyse_trading_date_range_index_Q3 = mcal.date_range(nyse_trading_date_range_Q3, frequency='1D')\
        .strftime('%Y-%m-%d')\
        .tolist()
    trading_Q4_dates.append(nyse_trading_date_range_index_Q3[-1])

    # Configure range of H2
    nyse_trading_date_range_Q4 = nyse.schedule(y, z)
    nyse_trading_date_range_index_Q4 = mcal.date_range(nyse_trading_date_range_Q4, frequency='1D')\
        .strftime('%Y-%m-%d')\
        .tolist()
    trading_end_dates.append(nyse_trading_date_range_index_Q4[-1])

    # Pull relevant price data for given Q2
    prices_dataframe_Q1 = prices.loc[datetime.strptime(str(nyse_trading_date_range_index_Q1[0]), '%Y-%m-%d'):
                                     datetime.strptime(str(nyse_trading_date_range_index_Q1[-1]), '%Y-%m-%d')]
    ...
    prices_dataframe_Q4 = prices.loc[datetime.strptime(str(nyse_trading_date_range_index_Q4[0]), '%Y-%m-%d'):
                                     datetime.strptime(str(nyse_trading_date_range_index_Q4[-1]), '%Y-%m-%d')]

    # Calculate efficient frontier with given covariance matrix and expected returns for Q2
    prices_expected_returns_Q1 = expected_returns.ema_historical_return(prices_dataframe_Q2)
    covariance_matrix_Q1 = quarterly_cov(nyse_trading_date_range_index_Q1[0], prices_expected_returns_Q1, tickers, length)
    ef_Q1 = EfficientFrontier(prices_expected_returns_Q1, covariance_matrix_Q1)
    ...
    covariance_matrix_Q2 = quarterly_cov(nyse_trading_date_range_index_Q2[0], prices_expected_returns_Q2, tickers, length)
    ...
    covariance_matrix_Q3 = quarterly_cov(nyse_trading_date_range_index_Q3[0], prices_expected_returns_Q3, tickers, length)
    ...
    prices_expected_returns_Q4 = expected_returns.ema_historical_return(prices_dataframe_Q4)
    covariance_matrix_Q4 = quarterly_cov(nyse_trading_date_range_index_Q4[0], prices_expected_returns_Q4, tickers, length)
    ef_Q4 = EfficientFrontier(prices_expected_returns_Q4, covariance_matrix_Q4)

    # Optimise portfolio and give weights for Q2
    raw_weights_Q1 = ef_Q1.nonconvex_objective(
        objective_functions.sharpe_ratio,
        objective_args=(ef_Q1.expected_returns, ef_Q1.cov_matrix),
        weights_sum_to_one=True,
    )
    cleaned_weights_Q1 = ef_Q1.clean_weights()
    ...
    raw_weights_Q4 = ef_Q4.nonconvex_objective(
        objective_functions.sharpe_ratio,
        objective_args=(ef_Q4.expected_returns, ef_Q4.cov_matrix),
        weights_sum_to_one=True,
    )
    cleaned_weights_Q4 = ef_Q4.clean_weights()

    # Append weights to dataframe 'weights'
    cleaned_weights_Q1 = dict(cleaned_weights_Q1)
    cleaned_weights_Q2 = dict(cleaned_weights_Q2)
    cleaned_weights_Q3 = dict(cleaned_weights_Q3)
    cleaned_weights_Q4 = dict(cleaned_weights_Q4)
    weights = weights.append(dict(cleaned_weights_Q1), ignore_index=True)
    ...
    weights = weights.append(dict(cleaned_weights_Q4), ignore_index=True)

    # Get allocation in shares
    latest_prices_Q1 = get_latest_prices(prices_dataframe_Q1)
    latest_prices_Q2 = get_latest_prices(prices_dataframe_Q2)
    latest_prices_Q3 = get_latest_prices(prices_dataframe_Q3)
    latest_prices_Q4 = get_latest_prices(prices_dataframe_Q4)
    da_Q1 = DiscreteAllocation(cleaned_weights_Q1, latest_prices_Q1, total_portfolio_value=portfolio_value)
    allocation_Q1, leftover_Q1 = da_Q1.lp_portfolio()
    da_Q2 = DiscreteAllocation(cleaned_weights_Q2, latest_prices_Q2, total_portfolio_value=portfolio_value)
    allocation_Q2, leftover_H1 = da_Q2.lp_portfolio()
    ...
    da_Q4 = DiscreteAllocation(cleaned_weights_Q4, latest_prices_Q4, total_portfolio_value=portfolio_value)
    allocation_Q4, leftover_H2 = da_Q4.lp_portfolio()
    allocation_shares = allocation_shares.append(dict(allocation_Q1), ignore_index=True)
    ...
    allocation_shares = allocation_shares.append(dict(allocation_Q4), ignore_index=True)

# Clean up weights dataframe
trading_dates_draft = list(zip(trading_start_dates, trading_Q2_dates, trading_mid_dates, trading_Q4_dates))
trading_dates_final = []
...
# Bug Hunting
daily_weights.to_csv('test1.csv')
daily_ret.to_csv('test2.csv')
...
# Quantstats
qs.extend_pandas()
qs.reports.metrics(daily_weights_returns['Daily Pct Return'], mode='full')
